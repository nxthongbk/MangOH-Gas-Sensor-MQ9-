 ...
 * - An actuator's main output resource must be called "value".
 * - The name of the "value" resource's parent entry is the name of the sensor or actuator.
 * - All output resources under the same parent as a "value" resource are for related settings.
 *
 * Furthermore, some conventions exist for settings related to sensors and actuators:
 * - If a boolean output resource called "enable" appears next to (under the same parent as)
 *   the "value" resource, then it can be used by administrator apps to disable the sensor
 *   or actuator (by setting that output to "false").
...
/app
  ...
  +--/hvac
      |
      +--/value = the HVAC system's temperature setpoint output
      |
