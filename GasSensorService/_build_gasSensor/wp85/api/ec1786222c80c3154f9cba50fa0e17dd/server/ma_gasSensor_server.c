...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("ma_gasSensor_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("ma_gasSensor_ServerHandlers", 3);
...
static void Handle_ma_gasSensor_Read
(
...
)
{
    ...
    double *readingPtr = &readingBuffer;
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_ma_gasSensor_Read : Handle_ma_gasSensor_Read(msgRef); break;
